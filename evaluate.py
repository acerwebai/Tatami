    ...
    with g.as_default(), g.device(device_t), \
        ...
        for i in range(num_iters):
            ...
            for j, path_out in enumerate(curr_batch_out):
                ...
                #samual add for add_shapes=True 20190116 start
                frozen_graph_def = tf.graph_util.convert_variables_to_constants(sess, sess.graph.as_graph_def(add_shapes=True), ["output"])
                pa = checkpoint_dir + '/evaluate_convert_frozen.pb'
                with open(pa, 'wb') as f:
                    f.write(frozen_graph_def.SerializeToString())
                #samual add for add_shapes=True 20190116 end
